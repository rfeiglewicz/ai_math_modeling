// Exhaustive Verilator equivalence test for bf16_expe_cut.
//
// Drives every one of the 65536 BF16 payloads through the AXI-Stream interface
// and compares the RTL output against the C++ model bf16_expe_cut_approx(),
...
static void tick(Vbf16_expe_cut* dut) {
    dut->clk = 1;
    dut->eval();
    dut->clk = 0;
    dut->eval();
...
int main(int argc, char** argv) {
    ...
    for (uint32_t raw = 0; raw <= 0xFFFF; ++raw) {
        ...
        queue.push_back({input, bf16_expe_cut_approx(input)});
    ...
    std::cout << "Checked: " << checked << "\nMismatches: " << mismatches << '\n'
              << "Table payload: " << bf16_expe_cut::PAYLOAD_BITS << " bits\n"
