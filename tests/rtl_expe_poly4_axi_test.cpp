// =============================================================================
// rtl_expe_poly4_axi_test.cpp
// Verilator testbench -- AXI-Stream protocol & backpressure for bf16_expe_poly4.
//
...
//
// This variant has a 7-deep pipeline (decompose + align + 4 DSP steps +
// output register), so it stresses the enable chain harder than the 4-deep
// cut-point ladder does.
//
// Build & run: make rtl_expe_poly4_axi_test
// =============================================================================
...
TickResult axi_tick(Vbf16_expe_poly4* dut) {
    TickResult r;
    dut->eval();
...
void reset_dut(Vbf16_expe_poly4* dut) {
    dut->clk = 0;
    dut->rst_n = 0;
    dut->s_axis_tvalid = 0;
    dut->s_axis_tdata = 0;
...
// ---------------------------------------------------------------------------
uint64_t run_stream(Vbf16_expe_poly4* dut, const char* test, int count,
    ...
    while ((next_index < count || pending || !queue.empty())
        ...
        if (r.input_accepted) {
            queue.push_back({pend_data, bf16_expe_poly4_approx(pend_data)});
            pending = false;
            ++next_index;
            progressed = true;
        }
...
int main(int argc, char** argv) {
    ...
    std::cout << "AXI-Stream protocol tests for bf16_expe_poly4\n"
              << "---------------------------------------------\n";
