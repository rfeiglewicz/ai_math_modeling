...
namespace bf16_cfg {
    ...
    /** @brief Multiplication Result Format (mant_src * log2e) */
    ...
    constexpr int MULT_I = IN_I + COEFF_I;
    constexpr int MULT_F = IN_F + COEFF_F;
    ...
    /** @brief Polynomial output format. */
    ...
    constexpr int HIDDEN_BIT_IDX = TARGET_MANT_W;    // Index of the hidden bit

    /** @brief Input conversion parameters for decomposing x into integer and fractional parts. */
    constexpr int IN_CONV_INT_W = INPUT_MAX_EXP + MANT_MULT_I;
...
inline PolyResult bf16_exp2_poly(mant_t mant_val) {
    ...
    PolyResult result;
    // Calculate exponent relative to format
...
inline FPRaw bf16_exp2_core_approx(const FPRaw& input_parts, bool base2 = true) {
    ...
    // Convert input to fixed-point format
    // Unified format
