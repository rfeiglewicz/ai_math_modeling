...
// Rounds the Q1.18 polynomial result to BF16 precision, round-to-nearest-even,
// including the subnormal path.
//
// Structurally identical to bf16_round; only the widths differ, and they are
// what makes it cheap:
//
//   poly_mantissa   59 bits -> 19 bits
//   BASE_SHIFT      51      -> 11
//   sticky mask     59 bits -> 19 bits (3 groups instead of 8)
//   variable shift  0..59   -> 0..19 for normals
//
// Steps (matches bf16_exp2_optim_core_approx in the C++ model):
//   1. final_exponent = poly_exponent + exponent_bias
//   2. subnormal if final_exponent < BF16_MIN_EXP
//   3. shift_val = BASE_SHIFT + subnormal_extra
//   4. extract LSB / guard / sticky
//   5. round_up = guard && (lsb || sticky)
//   6. shift the mantissa down, add round_up
//   7. carry fixup
//   8. assemble the fp_raw_t
//
// fp_raw_t comes from bf16_exp2_pkg by explicit qualification: the two
// packages cannot be wildcard-imported together (they share many names with
// different values), but the decomposed-BF16 struct is format plumbing and is
// the same in both cores.
// =============================================================================

module bf16_exp2_optim_round
...
#(
    ...
    // EXTRA_STAGES cuts it:
    //   1 - after the shift amount is known
    //   2 - additionally after the shift + round increment
    ...
    input  logic                            rst_n,
    input  logic                            pipe_en,
    input  logic [POLY_OUT_W-1:0]           poly_mantissa,   // Q1.18
    input  logic signed [8:0]               poly_exponent,
    input  logic signed [IN_CONV_INT_W-1:0] exponent_bias,
    output bf16_exp2_pkg::fp_raw_t          rounded_fp
);
    ...
    //
    // The sticky mask is written as a per-bit compare
    //     mask[i] = shift > i+1
    // rather than (1 << (shift-1)) - 1, which would synthesise as a borrow
    // chain. It is also uniformly correct at both edges: shift <= 1 gives an
    // all-zero mask and shift >= POLY_OUT_W gives an all-ones mask, which is
    // the deep-subnormal case where everything is sticky.
    // -------------------------------------------------------------------------
    logic                  lsb_bit;
    logic                  guard_bit;
    logic                  sticky_bit;
    ...
    bf16_exp2_pkg::fp_raw_t rounded_comb;
...
endmodule : bf16_exp2_optim_round
